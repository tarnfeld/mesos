...
class PluggableContainerizerTest : public MesosTest {};


class TestPluggableContainerizer : public slave::PluggableContainerizer
{
public:
  TestPluggableContainerizer(slave::Flags flags)
  : PluggableContainerizer(flags) {}
      ...
      bool checkpoint)
  {
    ...
    return PluggableContainerizer::launch(
        containerId,
        taskInfo,
        executorInfo,
        directory,
...
TEST_F(PluggableContainerizerTest, Launch)
{
  ...
  TestPluggableContainerizer testContainerizer(flags);
  ...
  do {
    ...
    Future<ResourceStatistics> usage = testContainerizer.usage(containerId.get());
    AWAIT_READY(usage);
  ...
  EXPECT_EQ(statistics.cpus_limit(), cpus.get()); // Deal with command executor over-commit.
